...
class Notes(commands.Cog):
    ...
    async def notelist(self, ctx):
        rows = await self.bot.db.fetch(f"SELECT id, title, Content FROM notes WHERE notes.Userid=$1", str(ctx.author.id))
    ...
    async def noteadd(self, ctx, title, *, content):
        await self.bot.db.execute("INSERT INTO notes(Userid, Title, Content) VALUES ($1, $2, $3)", str(ctx.author.id), str(title), str(content))
    ...
    async def noteremove(self, ctx, content: int):
        rows = await self.bot.db.fetch("SELECT * FROM notes WHERE notes.userid=$1 AND notes.id=$2", str(ctx.author.id), content)
        if len(rows) == 0:
            return await ctx.send("That note doesn't exist")
        await self.bot.db.execute("DELETE FROM notes WHERE notes.userid=$1 AND notes.id=$2", str(ctx.author.id), content)
        await ctx.send("Note removed")
        
     
    @commands.group(invoke_without_command=True, description="See your todo list")
    async def todo(self, ctx):
        rows = await self.bot.db.fetch(f"SELECT id, content, Status FROM todo WHERE todo.Userid=$1", str(ctx.author.id))
    ...
    async def todostart(self, ctx, *, content: int):
        rows = await self.bot.db.fetch(f"SELECT * FROM todo WHERE todo.Userid=$1 and todo.ID=$2", str(ctx.author.id), content)
        if len(rows) == 0:
            return await ctx.send("That is not on your todo list")
        await self.bot.db.execute(f"UPDATE todo SET Status='Started' WHERE todo.userid=$1 AND todo.id=$2", str(ctx.author.id), content)
    ...
    async def todocomplete(self, ctx, *, content: int):
        ...
        if len(rows) == 0:
            return await ctx.send("That is not on your todo list")
        await self.bot.db.execute(f"UPDATE todo SET Status='Complete' WHERE todo.userid=$1 and todo.id=$2", str(ctx.author.id), content)
    ...
    async def todoremove(self, ctx, *, content: int):
        ...
        if len(rows) == 0:
            return await ctx.send("That is not on your todo list")
        await self.bot.db.execute(f"DELETE FROM todo WHERE todo.userid=$1 AND Todo.id=$2", str(ctx.author.id), content)
